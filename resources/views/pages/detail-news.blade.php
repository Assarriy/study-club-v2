            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <section class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden reveal">
                    <div class="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-slate-100">
                        ...
                        @if($club->achievements->isNotEmpty())
                            <span class="text-[10px] font-bold px-2.5 py-1 rounded-full flex-shrink-0" style="background: var(--accent-light); color: var(--accent-text);">{{ $club->achievements->count() }} prestasi</span>
                        @endif
                    </div>

                    @forelse($club->achievements as $i => $achievement)
                        <div class="flex items-center gap-4 px-5 sm:px-6 py-4 border-b border-slate-50 last:border-b-0 hover:bg-slate-50/60 transition-colors group">
                ...
                <section class="bg-white rounded-3xl border border-slate-200/80 shadow-sm overflow-hidden reveal reveal-delay-1">
                    ...
                    @forelse($club->posts as $post)
                        <article class="flex gap-4 px-5 sm:px-6 py-4 border-b border-slate-50 last:border-b-0 hover:bg-slate-50/60 transition-colors group">
